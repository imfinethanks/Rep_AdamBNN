...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1):
        ...
        self.move11 = LearnableBias(inplanes*2)
        self.binary_3x3= binaryconv3x3(inplanes*2, int(inplanes/2), stride=stride)
        self.bn1 = norm_layer(inplanes*2)

        self.move12 = LearnableBias(inplanes*2)
        self.prelu1 = nn.PReLU(inplanes*2)
        self.move13 = LearnableBias(inplanes*2)

        self.move21 = LearnableBias(inplanes*2)

        if inplanes == planes:
            self.binary_pw = binaryconv1x1(inplanes*2, int(planes/2))
            self.bn2 = norm_layer(int(planes*2))
        else:
            self.binary_pw_down1 = binaryconv1x1(inplanes*2, int(inplanes/2))
            self.binary_pw_down2 = binaryconv1x1(inplanes*2, int(inplanes/2))
            self.bn2_1 = norm_layer(int(inplanes*2))
            self.bn2_2 = norm_layer(int(inplanes*2))

        self.move22 = LearnableBias(planes*2)
        self.prelu2 = nn.PReLU(planes*2)
        self.move23 = LearnableBias(planes*2)
    ...
    def forward(self, x):
        ...
        if self.inplanes == self.planes:
            ...
            out2_1 = out2_1.repeat(1, 4,1,1)
            out2_2 = out2_2.repeat(1, 4,1,1)
