...
#wandb.init(project="react_a_imagenet", entity="llll",name=time.strftime('%Y_%m_%d_%H:%M:%S'),group="react_a_imagenet_x2")
wandb.init(project="react_a_imagenet", entity="llll",name='2022_01_06_16:57:58',group="react_a_imagenet_x2",id='2hxequkg',resume='must')
...
def main():
    ...
    # model_teacher = bl.BalancedDataParallel(0,model_teacher, dim=0).cuda()
    ...
    # model_student = bl.BalancedDataParallel(22,model_student, dim=0).cuda()
    ...
    valdir = os.path.join(args.data, 'ILSVRC2012_img_val_fl')
...
def train(epoch, train_loader, model_student, model_teacher, criterion, optimizer, scheduler):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        wandb.log({"train_top1.avg": top1.avg})
        wandb.log({"train_top5.avg": top5.avg})
...
def validate(epoch, val_loader, model, criterion, args):
    ...
    wandb.log({"test_top1.avg": top1.avg})
    wandb.log({"test_top5.avg": top5.avg})
